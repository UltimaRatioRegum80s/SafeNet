// Manual location data for Southern African countries
...
// Neighbourhood data for major cities
const NEIGHBOURHOOD_DATA: Record<string, string[]> = {
...
export function getCitiesByCountry(countryName: string): LocationData[] {
  const cities = LOCATION_DATA[countryName as keyof typeof LOCATION_DATA];
  if (!cities) {
    return [];
  }
...
export function getNeighbourhoodsByCity(cityName: string, countryName: string): NeighbourhoodData[] {
  const neighbourhoods = NEIGHBOURHOOD_DATA[cityName];
  if (!neighbourhoods) {
    return [];
  }
